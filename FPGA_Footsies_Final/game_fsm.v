module game_fsm (
    ...
    output reg [1:0] winner,        // 00: no winner, 01: P1, 10: P2, 11: tie/timeout
    ...
    // Key edge detection
    reg [3:0] keys_prev;
    wire [3:0] key_pressed;  // Rising edge detection
    ...
    // Detect rising edge (key press, not key hold)
    ...
    assign p1_wins = game_over && (health_p2 == 0) && (health_p1 != 0);
    assign p2_wins = game_over && (health_p1 == 0) && (health_p2 != 0);
    ...
    // LED output (show menu LEDs in menu state, or status in other states)
    assign leds = (current_state == MENU_STATE) ? menu_leds : 
                  {6'b000000, winner, current_state[1:0]};
